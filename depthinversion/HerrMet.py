...
import os, imp
import numpy as np
from labex.tools.generic_nobspy import readargv, minmax
from displininv import Parameterizer, resultdisplay, r43, Datacoder_log, makedatacoder, Theory, surf96reader
...
def overdisp(ms, overwaves, overtypes, overmodes, overfreqs, **mapkwargs):
    ...
    def fun(mms):
        ...
        try:
            ...
            overvalies = np.nan * np.ones(len(overwaves))
    ...
    print
# -------------------------------------
class resultdisplay1(resultdisplay):
    def __init__(self, fig = None, targetfile = None):
        ...
        if targetfile is None:
            self.g = grapher(self.fig, 4, 6)
            self.axvp = self.g(":", 0, title="$V_P$", ylabel="depth (km)")
            self.axvs = self.g(":", 1, title="$V_S$", sharey=self.axvp)  # , sharex = self.axvp)
            self.axpr = self.g(":", 2, title=r"$V_P/V_S$", sharey=self.axvp)
            self.axrh = self.g(":", 3, title=r"$\rho$", sharey=self.axvp)

            self.axru0 = self.g(0, 4, title="RU0", ylabel="velocity (km/s)")
            self.axru1 = self.g(1, 4, title="RU1", ylabel="velocity (km/s)", sharex=self.axru0, sharey=self.axru0)
            self.axrc0 = self.g(0, 5, title="RC0", ylabel="velocity (km/s)", sharex=self.axru0, sharey=self.axru0)
            self.axrc1 = self.g(1, 5, title="RC1", ylabel="velocity (km/s)", sharex=self.axru0, sharey=self.axru0)
            ...
            self.g = grapher(self.fig, Ndisp, 5)
            self.axvp = self.g(":", 0, title="$V_P\,(km/s)$", ylabel="depth (km)")
            self.axvs = self.g(":", 1, title="$V_S\,(km/s)$", sharey=self.axvp)  # , sharex = self.axvp)
            self.axpr = self.g(":", 2, title=r"$V_P/V_S$", sharey=self.axvp)
            self.axrh = self.g(":", 3, title=r"$\rho\,(g/cm^3)$", sharey=self.axvp)

            self.axdisp = []
            share = None
            for n, (w, t, m)  in enumerate(s.wtm()):
                ax = self.g(n, 4, title="%s%s%d" % (w, t, m),
...
--param      int float generate a template parameter file to custom, need the number of layers
