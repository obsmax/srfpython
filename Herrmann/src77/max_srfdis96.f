c mes modifs
c le programme prend ses entrees dans stdin, elles correspondent aux sorties de 
c la version modifiee de srfpre96 vers stdout
c sorties vers stdout
c tableau:
c    premiere colonne  : 1 pour LOVE, 2 pour RAYLEIGH, 0 pour un mode manquant (ignorer le reste de la ligne dans ce cas)
c    seconde  colonne  : numero de mode, 0=fondamental,...
c    troisieme et 4eme colonnes : periode en s
c        si la 4eme colonne vaut 0, alors la 3eme colonne est la periode absolue
c        sinon la 3eme colonne est diminuee d un facteur 1-h et la 4eme est augmentee d'un fact. 1+h 
c    5eme et 6eme : vitesse de phase correspondant aux colonnes 3 et 4
c----------------------------------------------------------------------c
c                                                                    c
